...
class SkinDownloadState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		coolText.text = "Skin Pack Installer";
		coolText.screenCenter(X);
		coolText.y += 200;
		coolText.cameras = [camHUD];
		add(coolText);

		var coolButton = new FlxButton(0, 0, "Install Skin Pack", function()
		{
			if (idInputText.text != "" && !alreadyPressed)
			{
				alreadyPressed = true;
				downloadInputtedSkin();
			}
			else
			{
				if (!alreadyPressed)
				{
					coolText.text = "No Skin Pack Found!";
				}
			}
		});
	...
	public static function downloadInputtedSkin()
	{
				...
				for (folder in FileSystem.readDirectory(coolFile.replace('.zip', '') + '/'))
				{
					if (FileSystem.exists('mods/$folder'))
					{
						if (FileSystem.readDirectory('mods/$folder/') != null
							&& !FileSystem.readDirectory('mods/$folder/').contains('_polymod_meta.json'))
						{
							coolText.text = 'This Is Not A Mag Engine Skin Pack!';
